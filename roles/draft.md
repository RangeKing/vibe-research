...
- Lead titles, abstracts, Results subheadings, Discussion openings, and cover-letter claims with the strongest defensible scientific product rather than the weakest limitation. Make quality control and uncertainty visible as confidence architecture around the result, not as the substitute for the result, unless the piece is explicitly a methods or data-quality paper.
- For model, prediction, scenario, or spatial-map papers, label direct observation, model prediction, sensitivity analysis, extrapolation, and evidence-insufficient areas before drafting stronger result language. The publishable product may be a prediction map, confidence-graded risk assessment, scenario transition, or management priority, but the text must not let readers confuse prediction with direct observation.
...
- Do not make Results subheadings sound like workflow steps such as data cleaning, screening, or QC unless the result is truly methodological. Convert them into findings: prediction, risk shift, threshold, mechanism, spatial boundary, scenario response, or decision implication, with the evidence boundary still visible.
