...
- Before positioning a work for any target venue or target format, test whether the central claim is credible at the claim level: direct evidence, indirect evidence, validation class, uncertainty propagation, figure/table support, and scope boundary.
- Use `references/research-credibility-story-kernel.md` and `templates/score_cap_card.md` for generic venue-fit scoring, even when no named journal is involved.
- Establish content-type identity before scoring fit. If the package is labeled Article, Analysis, Review, Perspective, or another format, judge it against that format's core promise before giving target-journal readiness points.
- For Nature Climate Change or Nature-family targets, use `references/nature-climate-change-editorial-gate.md` before fit optimism, cover-letter positioning, or Article readiness scoring.
- Apply the Article vs Analysis forced decision: if the central metric is constructed, ordinal, diagnostic, proxy-based, or threshold-weighted; most evidence comes from existing public datasets; and the headline claim is conditional on index design, thresholds, component weights, or source-data reconciliation, default the content type to Analysis unless at least two Article recovery criteria are visibly met.
...
- Treat every score as a heuristic gap diagnosis. Include hard caps, blockers to next band, fastest credible improvement, and what would not raise the score.
...
- For high-impact targets, require the desk-rejection memo, hostile reviewer panel, claim-figure-source-data truth table, rendered figure inspection, display-item budget, and score cap table before reporting that desk rejection is unlikely.
- Use any credible external editorial viability benchmark as a calibration constraint. If an outside critique says the same package is 58/100 as Article but 65/100 if reframed as Analysis, the local journal scorecard must explain the Article-vs-Analysis gap, apply the content-type cap, and avoid reporting a higher Article score unless the underlying Article identity has been rebuilt and rechecked.
